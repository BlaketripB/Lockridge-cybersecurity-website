...
if (checklistQEl) {
  ...
  // Single card element — swapped in place, never stacks
  const card = document.createElement('div');
  card.className = 'checklist-q';
  checklistQEl.appendChild(card);
    ...
    return `
      <div class="checklist-q-inner">
        ...
        <div class="checklist-buttons">
          <button class="check-btn check-btn-yes" data-ans="yes">✓ Yes</button>
          <button class="check-btn check-btn-no"  data-ans="no">✗ No</button>
        </div>
      </div>
    `;
  }
  ...
  function bindButtons() {
    card.querySelectorAll('.check-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // Update dots
        const prevDot = document.getElementById('dot-' + currentQ);
        if (prevDot) { prevDot.classList.remove('active'); prevDot.classList.add('done'); }
        const nextDot = document.getElementById('dot-' + (currentQ + 1));
        ...
        setTimeout(() => {
          ...
          // Load new content, position off-screen right (no transition)
          card.innerHTML = getQuestionHTML(currentQ);
          card.classList.add('slide-in');

          // Force reflow so browser registers the off-screen position
  ...
  // Show first question
  card.innerHTML = getQuestionHTML(0);
